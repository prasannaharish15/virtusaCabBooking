    ...
    <div class="fixed top-4 right-4 z-50 space-y-2">
      <div
        *ngFor="let notification of notifications; trackBy: trackByFn"
        ...
        class="min-w-[300px] max-w-md px-6 py-4 rounded-lg shadow-lg text-white flex items-center space-x-3 cursor-pointer"
        ...
        <button
          ...
          class="flex-shrink-0 ml-2 text-white hover:text-gray-200 transition">
...
export class NotificationComponent implements OnInit, OnDestroy {
  notifications: Array<NotificationMessage & { id: number }> = [];
  ...
  removeNotification(notification: NotificationMessage & { id: number }): void {
    this.notifications = this.notifications.filter(n => n.id !== notification.id);
  }

  trackByFn(index: number, item: NotificationMessage & { id: number }): number {
    return item.id;
  }
}
